...
- **Update the README index:** The root [`README.md`](README.md) is the repo's public entry point and indexes the `resources/` folders. When you add or rename a `resources/<slug>/` folder — or publish the post that accompanies one — add or update its row in the README's resource table, and swap any _Coming soon_ for the live article link.
